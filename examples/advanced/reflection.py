...
def debate_reflection(topic: str) -> str:
    ...
    position_a = complete(MODEL, [user(f"Argue strongly FOR: {topic}")], api_key=API_KEY).content
    print(f"For:\n{position_a}\n")

    position_b = complete(
        MODEL, [user(f"Argue strongly AGAINST: {topic}")], api_key=API_KEY
    ).content
    print(f"Against:\n{position_b}\n")
